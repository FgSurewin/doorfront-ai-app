...
export const useImages = (
  ...
  const getImages = async () => {
    setLoading(true);
    try {
      const response = await fetchAllImages(); // Fetch all images without pagination
      if (response.data) {
        setTotalImages(response.data.length); // Store total images count
        setImages(response.data); // Store full dataset
      } else {
        setImages([]);
      }
    } catch (error) {
  ...
  return { images, loading, totalImages, getImages, 
    handleDeleteClick, handleConfirmDelete, handleCancelDelete, openDialog,
    handleOpenImageInfo, selectedImage,
