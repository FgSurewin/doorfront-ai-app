    ...
    async function innerFunc() {
      try {
        ...
        if (allReferredUsers.data) {
          for (let i = 0; i < allReferredUsers.data.length; i++) {
            let BR = false;
            ...
            if (currentUserScore.data.score >= 1 && allReferredUsers.data[i].bonusReceived !== true) {
              ...
              const t1 = await updateReferredUserBonus({
    ...
    <div>
      ...
      <Box sx={{display: 'flex', flexDirection: 'row', paddingTop: 3, width: '100%'}}>
        <Box sx={{width: '20%', paddingTop: 2, paddingRight: 5}}>
          <Tabs value={value} onChange={handleTabChange} aria-label="profile" orientation="vertical"
                sx={{backgroundColor: '#f9edda'}}>
        ...
        <TabPanel value={value} index={0}>
          <Box>
            <Grid container rowSpacing={3}>
              <Grid item xs={4} sx={gridTitle} textAlign='center'><Typography marginTop={1} sx={{
                display: "flex",
                flexDirection: "row",
                fontWeight: 'bold'
              }} fontSize={18}>Nickname:</Typography></Grid>
              <Grid item xs={8} sx={gridContent} textAlign='center' display="flex" justifyContent={"center"}><Typography
                marginTop={1} sx={{display: "flex", flexDirection: "row", alignSelf: "center"}}
                fontSize={18}>{userInfo.nickname}</Typography></Grid>
              <Grid item xs={4} sx={gridTitle} textAlign='center'><Typography marginTop={1} sx={{
                display: "flex",
                flexDirection: "row",
                fontWeight: 'bold'
              }} fontSize={18}>Score:</Typography></Grid>
              <Grid item xs={8} sx={gridContent} textAlign='center' display="flex" justifyContent={"center"}><Typography
                marginTop={1} sx={{display: "flex", flexDirection: "row", alignSelf: "center"}}
                fontSize={18}>{userScore.score}</Typography></Grid>
              <Grid item xs={4} sx={gridTitle} textAlign='center'><Typography marginTop={1} sx={{
                display: "flex",
                flexDirection: "row",
                fontWeight: 'bold'
              }} fontSize={18}>Created:</Typography></Grid>
              <Grid item xs={8} sx={gridContent} textAlign='center' display="flex" justifyContent={"center"}><Typography
                marginTop={1} sx={{display: "flex", flexDirection: "row", alignSelf: "center"}}
                fontSize={18}>{userScore.create}</Typography></Grid>
              <Grid item xs={4} sx={gridTitle} textAlign='center'><Typography marginTop={1} sx={{
                display: "flex",
                flexDirection: "row",
                fontWeight: 'bold'
              }} fontSize={18}>Labeled:</Typography></Grid>
              <Grid item xs={8} sx={gridContent} textAlign='center' display="flex" justifyContent={"center"}><Typography
                marginTop={1} sx={{display: "flex", flexDirection: "row", alignSelf: "center"}}
                fontSize={18}>{userScore.label}</Typography></Grid>
              <Grid item xs={4} sx={gridTitle} textAlign='center'><Typography marginTop={1} sx={{
                display: "flex",
                flexDirection: "row",
                fontWeight: 'bold'
              }} fontSize={18}>Reviewed:</Typography></Grid>
              <Grid item xs={8} sx={gridContent} textAlign='center' display="flex" justifyContent={"center"}><Typography
                marginTop={1} sx={{display: "flex", flexDirection: "row", alignSelf: "center"}}
                fontSize={18}>{userScore.review}</Typography></Grid>
              <Grid item xs={4} sx={gridTitle} textAlign='center'><Typography marginTop={1} sx={{
                display: "flex",
                flexDirection: "row",
                fontWeight: 'bold'
              }} fontSize={18}>Modified:</Typography></Grid>
              <Grid item xs={8} sx={gridContent} textAlign='center' display="flex" justifyContent={"center"}><Typography
                marginTop={1} sx={{display: "flex", flexDirection: "row", alignSelf: "center"}}
                fontSize={18}>{userScore.modify}</Typography></Grid>
              <Grid item xs={4} sx={gridTitle} textAlign='center'><Typography marginTop={1} sx={{
                display: "flex",
                flexDirection: "row",
                fontWeight: 'bold'
              }} fontSize={18}>Bonus:</Typography></Grid>
              <Grid item xs={8} sx={gridContent} textAlign='center' display="flex" justifyContent={"center"}><Typography
                marginTop={1} sx={{display: "flex", flexDirection: "row", alignSelf: "center"}}
                fontSize={18}>{userScore.bonus}</Typography></Grid>
              {readLocal("contest" as LocalStorageKeyType) !== null && readLocal("contest" as LocalStorageKeyType) !== "" &&
                  <>
                      <Grid item xs={4} sx={gridTitle} textAlign='center'><Typography marginTop={1} sx={{
                        ...
                        flexDirection: "row",
                        fontWeight: 'bold'
                      }} fontSize={18}>Contest Score:</Typography></Grid>
                      <Grid item xs={8} sx={gridContent} textAlign='center' display="flex"
                            justifyContent={"center"}><Typography marginTop={1} sx={{
                        display: "flex",
                        flexDirection: "row",
                        alignSelf: "center"
                      }} fontSize={18}>{userScore.contestScore}</Typography></Grid>
                  </>
              }
            </Grid>
          ...
          <Box sx={{width: '75%'}}>
            <Grid container rowSpacing={3}>
              ...
              <Grid item xs={4} sx={gridTitle} textAlign='center'><Typography marginTop={1} sx={{
                ...
                flexDirection: "row",
                fontWeight: 'bold'
              }} fontSize={18}>Change Password:</Typography></Grid>
    ...
    */}
        ...
        <TabPanel value={value} index={2}>
          <Box>
            ...
            <Box sx={{display: 'flex', flexDirection: 'row', border: .5}}>
              <Box sx={{mx: 11}}/>
              <TwitterShareButton url="DoorFront.org"
                                  title={"Try beating my score of " + userScore.score + '!\n\n'}><TwitterIcon/>
              </TwitterShareButton>
              <Box sx={{mx: 4}}/>
              <FacebookShareButton url='DoorFront.org'
                                   hashtag={'#BeatMyScoreOf' + userScore.score + '! #accessibility'}><FacebookIcon/></FacebookShareButton>
              <Box sx={{mx: 4}}/>
              <LinkedinShareButton url='DoorFront.org' title={"Try beating my score of " + userScore.score + '!\n\n'}
                                   source="DoorFront.org"
                                   summary="Crowdsourcing App"><LinkedinIcon/></LinkedinShareButton>
              <Box sx={{mx: 2}}/>
            </ Box>
          </Box>
        </TabPanel>
        <TabPanel value={value} index={3}>
          ...
          <Grid container marginTop={2} spacing={.1}>
            <Grid item xs={6}>
              <Typography marginTop={1} sx={{display: "flex", flexDirection: "row"}} fontSize={18}>Referral
                Code</Typography>
            </Grid>
            <Grid item xs={6}>
              <TextField disabled size={"small"} sx={{
                display: "flex",
                flexDirection: "row",
                "& .MuiInputBase-input.Mui-disabled": {WebkitTextFillColor: "#000000"}
              }} defaultValue={ref}></TextField>
            </Grid>
            <Grid item xs={6}>
              <Typography marginTop={1} sx={{display: "flex", flexDirection: "row"}} fontSize={18}>Referral
                Link</Typography>
            ...
            <Grid item xs={4}>
              <TextField disabled size={"small"} sx={{
                ...
                flexDirection: "row",
                "& .MuiInputBase-input.Mui-disabled": {WebkitTextFillColor: "#000000"}
              }} defaultValue={window.location.host + "/login?ref=" + ref}></TextField>
            </Grid>
            <Grid item xs={2}>
              <Button
                size='large'
            ...
            <Grid item xs={6}>
              <Typography marginTop={1} sx={{display: "flex", flexDirection: "row", fontWeight: "bold"}} fontSize={18}
                          color={"black"}>User Referred</Typography>
            </Grid>
            <Grid item xs={6}>
              <Typography marginTop={1} sx={{display: "flex", flexDirection: "row", fontWeight: "bold"}} fontSize={18}
                          color={"black"}>Bonus Received</Typography>
            </Grid>
            {usersReferred.map((user, index) => (
              <React.Fragment key={user.userID}>
                <Grid item xs={6}>
                  <Typography marginTop={1} sx={{display: "flex", flexDirection: "row"}}
                              fontSize={18}>{usersReferred[index].userID}</Typography>
                </Grid>
                <Grid item xs={6}>
